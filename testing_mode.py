...
class Character:
    ...
    def attack_opponent(self, opponent, distance):
        print(f"{self.name} ({self.character_class}) attacking {opponent.name} ({opponent.character_class})")
        if distance <= self.range:
            ...
            print(f"{self.name} is out of range to attack.")
    ...
    def use_special_ability(self, opponent, distance):
        ...
        if self.character_class == "Fighter":
            ...
            return f"{self.name} casts Fireball, dealing {RED}{damage}{RESET} damage!"
        elif self.character_class == "Ranger":
            self.attack += 5
            self.special_ability_cooldown = 3
            ...
            self.attack += 10
            self.special_ability_cooldown = 3
            return f"{self.name} enters a Berserker Rage, increasing attack by 10!"
        elif self.character_class == "Rogue":
            if distance <= self.range:
...
def test_ranger():
    char1 = Character("Ranger1", "Ranger")
    char2 = Character("Opponent", "Fighter")
    battle(char1, char2)
...
def battle(char1, char2, silent=False):
    ...
    while char1.health > 0 and char2.health > 0:
        ...
        if defender.health <= 0:
            if defender.character_class == "Barbarian" and defender.rage_available:
                ...
                defender.health = random.randint(20, 40)
                if not silent:
    ...
    return {
        ...
        "loser_never_in_range": not loser_in_range,
            ...
            for class2 in classes:
                if class1 != class2:
                    ...
                    for i in range(1000):
                        ...
                        results[class1][class2]["loser_never_in_range"] += 1 if battle_result["loser_never_in_range"] else 0
    ...
    for class1 in classes:
        ...
        for class2 in classes:
            if class1 != class2:
                ...
                loser_never_in_range_percentage = (matchup_results["loser_never_in_range"] / 1000) * 100
                ...
                print(f"    Loser Never in Range: {loser_never_in_range_percentage:.2f}%")
